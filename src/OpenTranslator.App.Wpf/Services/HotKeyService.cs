...
public class HotKeyService : IDisposable
{
    ...
    public bool Start()
    {
        ...
        try
        {
            ...
            {
                Console.WriteLine($"[HotKeyService] CreateMessageWindow 失败: {LastError}");
                return false;
            }

            var ok = RegisterHotKeys();
            Console.WriteLine($"[HotKeyService] RegisterHotKeys 结果: {ok}, LastError={LastError}");
            return ok;
        }
        catch (Exception ex)
        {
            LastError = $"Start 异常: {ex.Message}";
            Console.WriteLine($"[HotKeyService] Start 异常: {ex}");
            return false;
        }
    }
    ...
    private bool RegisterHotKeys()
    {
        ...
        // 注册划词翻译热键
        if (TryParseHotKey(config.HotKeys.TranslateHotKey, out var mod1, out var vk1))
        {
            if (!RegisterSingleHotKey(NativeMethods.HOTKEY_ID_TRANSLATE, mod1 | NativeMethods.MOD_NOREPEAT, vk1))
                allSuccess = false;
        }
        else
        {
            Console.WriteLine($"[HotKeyService] 解析翻译热键失败: {config.HotKeys.TranslateHotKey}");
            allSuccess = false;
        }

        // 注册原地替换翻译热键
        if (TryParseHotKey(config.HotKeys.ReplaceHotKey, out var mod2, out var vk2))
        {
            if (!RegisterSingleHotKey(NativeMethods.HOTKEY_ID_REPLACE, mod2 | NativeMethods.MOD_NOREPEAT, vk2))
                allSuccess = false;
        }

        // 注册截图翻译热键 (预留)
        if (TryParseHotKey(config.HotKeys.ScreenshotHotKey, out var mod3, out var vk3))
        {
            if (!RegisterSingleHotKey(NativeMethods.HOTKEY_ID_SCREENSHOT, mod3 | NativeMethods.MOD_NOREPEAT, vk3))
                allSuccess = false;
        }

        // 注册词典查询热键 (预留)
        if (TryParseHotKey(config.HotKeys.DictionaryHotKey, out var mod4, out var vk4))
        {
            if (!RegisterSingleHotKey(NativeMethods.HOTKEY_ID_DICTIONARY, mod4 | NativeMethods.MOD_NOREPEAT, vk4))
                allSuccess = false;
        }
    ...
    /// <summary>
    /// 解析热键字符串，如 "Ctrl+Alt+Q" -> (MOD_CONTROL|MOD_ALT, VK_Q)
    /// </summary>
    ...
    private bool RegisterSingleHotKey(int id, uint modifiers, uint vk)
    {
        ...
        try
        {
            if (!NativeMethods.RegisterHotKey(_messageWindowHandle, id, modifiers, vk))
            {
                ...
                Console.WriteLine($"[HotKeyService] RegisterHotKey ID={id} 失败: Win32 错误 {error}");
                return false;
            }

            Console.WriteLine($"[HotKeyService] RegisterHotKey ID={id} 成功");
            return true;
        }
        catch (Exception ex)
        {
            LastError = $"RegisterHotKey (ID={id}) 异常: {ex.Message}";
            Console.WriteLine($"[HotKeyService] RegisterHotKey (ID={id}) 异常: {ex}");
            return false;
        }
    }
